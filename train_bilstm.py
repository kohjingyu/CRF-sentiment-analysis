...
models_set = ['baseline', 'seq2seq', 'attnseq2seq', 'bilstm_crf']  # What models are available
...
class POSTrainDataset(Dataset):
    ...
    def __init__(self, path, dataset_choice='EN', split=0.8, unk_chance=0.05, start_stop=False):
        self.path = path
...
def main():
    ...
    # Init Train Dataset
    posdataset = POSTrainDataset(data_dir, start_stop=True)
    ...
    print(f"Using model {models_set[model_choice]}")
    ...
    if model_choice == 0:
        ...
        model = BiLSTM_CRF(
            len(posdataset.wtoi), posdataset.ttoi, EMBEDDING_SIZE, HIDDEN_DIM, N_LAYERS)
        model.to(device)
        # Set up optimizer for Seq2Seq Model
        optimizer = optim.SGD(model.parameters(), lr=LEARNING_RATE, momentum=MOMENTUM, weight_decay=WEIGHT_DECAY)
