...
parser = argparse.ArgumentParser(description='Run a deep learning model with CRF for POS tagging. Example usage: python generate_bilstm_crf.py bilstm_h512_n2_lr0.001000_d0.5 --checkpoint_dir checkpoint/best')
...
parser.add_argument('--dataset_split', default="dev", type=str, help='dataset split to evaluate on (train / dev / test)')
parser.add_argument('--checkpoint_dir', default="checkpoint", type=str, help='where the checkpoint files are located')
parser.add_argument('--output_dir', default="preds", type=str, help='output directory to write predictions to')
...
class POSGenDataset(Dataset):
    ...
    def __init__(self, path, train_wtoi, train_ttoi, dataset_choice='EN', dataset_split='dev'):
        ...
        with open(self.path / self.dataset_choice / (str(dataset_split) + ".in"), encoding="utf-8") as f:
            data = []
            for line in f:
                # Strip newline
